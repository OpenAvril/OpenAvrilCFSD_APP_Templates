...
using OpenAvrilCFSD.ServerAssembly.Graphics.Cameras;
using OpenAvrilCFSD.ServerAssembly.Graphics.Renderables;

namespace OpenAvrilCFSD.ServerAssembly.Graphics.GameObjects
{
    public abstract class AGameObject
    {
        ...
        {
            /*
            float rot_X = OpenAvrilCFSD.ServerAssembly.Framework.GetGameServer().GetData().GetGame_Instance().Get_gameObjectFactory().Get_player().Get_Rotation().X + value.X;
            ...
            float rot_Y = OpenAvrilCFSD.ServerAssembly.Framework.GetGameServer().GetData().GetGame_Instance().Get_gameObjectFactory().Get_player().Get_Rotation().Y + value.Y;
            ...
            float rot_Z = OpenAvrilCFSD.ServerAssembly.Framework.GetGameServer().GetData().GetGame_Instance().Get_gameObjectFactory().Get_player().Get_Rotation().Z + value.Z;
            ...
            OpenAvrilCFSD.ServerAssembly.Framework.GetGameServer().GetData().GetGame_Instance().Get_gameObjectFactory().Get_player().Set_rotation(new Vector3(rot_X, rot_Y, rot_Z));
